export default function TrafficSourcesSection() {
  ...
  return (
    <section id="traffic" className="bg-[var(--bg-1)] py-24">
      <div className="mx-auto max-w-[1240px] px-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {verticals.map((v) => (
            <div
              key={v.id}
              className="group flex flex-col items-center gap-2 rounded-[14px] border border-white/5 bg-[var(--bg-card)] px-4 py-7 text-center transition-all duration-200 hover:border-blue-400 hover:bg-slate-800 hover:shadow-[0_14px_36px_rgba(62,114,236,0.14)] hover:-translate-y-1"
            >
              <div className="flex h-14 w-14 items-center justify-center rounded-[13px] bg-[var(--blue-dim)] text-xl flex-shrink-0">
                {v.emoji}
              </div>
              <h3 className="text-sm font-black text-white">{v.name}</h3>
              <p className="text-[0.72rem] text-[var(--t3)]">{v.desc}</p>
            </div>
          ))}
        </div>
      </div>
